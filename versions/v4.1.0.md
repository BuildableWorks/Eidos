...
- **`### Custom Properties`** — everything else the registry carries. The seed pre-populates it with a few useful defaults (a lifecycle `status`, the two dates, `tags`, and — scoped to `Specs` — `domain`, `depends_on`, and `type`), but Eidos does not depend on them; keep, scope, or drop any. Yours, and preserved across migration.
