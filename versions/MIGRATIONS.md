...
**One key rename, a type on each entry, and a walk of the root; then set `eidos_version: 5.3.0`.** In `.eidos/Framework.yaml`, rename `collections:` to `folders:` and add `type: collection` to every entry; nothing else on a collection entry changes. Then compare the root against the document. Every folder at the root other than `.eidos/` needs an entry: a folder of non-markdown files is `type: assets`, anything else the owner describes is `type: other`, and a folder that should not be there moves out. Every file at the root needs a `top_level` entry with a description the owner writes; offer an empty one and ask, never invent it. An entry with nothing behind it is dropped only after telling the owner. Hidden entries (`.git`, `.obsidian/`, `.gitignore`) are neither declared nor faulted. Inside each collection, every sub-folder is now a declared group and a group holds blueprints and nothing deeper: a sub-folder no group names is added to `grouping.groups` with the owner's description, and a folder inside a group is surfaced for the owner to flatten or promote. A non-markdown file inside a collection is not a blueprint and is left alone; offer to move images and documents into an `assets` folder but do not require it. Links and embeds that are not relative markdown paths (wikilinks, absolute paths) are surfaced; a check now verifies every path resolves. The Properties table, templates, roles, Vocabulary, regions, and plugins are untouched. The seeds ship an `assets` folder; an existing root gains one only if the owner wants it.

## 5.2.0 → 5.2.1

**Prose only: set `eidos_version: 5.2.1`; nothing else moves.** 5.2.1 stops requiring a root `README.md` and stops requiring that `top_level` lead with one. A root that has a README keeps it and keeps it listed first, which is still the recommendation and what every seed ships; a root without one is now conforming rather than missing something, and its `top_level` starts with whatever it has. No file, folder, property, or key changes, and a check reports nothing new.

## 5.1.0 → 5.2.0

**Additive: property options, and a tool's personal file. Add one line to `.eidos/.gitignore` and set `eidos_version: 5.2.0`; nothing else has to move.** 5.2.0 adds a sixth field to a property entry, `options`, closing a Text value (or a List's elements) to a declared, non-empty set. An entry without the key means what it always meant, so no blueprint changes and no entry has to gain one on the way up. Where a custom property's `meaning` lists its values in prose (a seed's `status` did, with "an off-list value warns" beside them), offer to move the list into `options` and leave `meaning` saying what the property is for; that is the owner's call, since it turns a sentence into a check. Do not add `options` to `variant` or to a grouping property: their sets are the collection's declared variants and groups, and the standard names them as the exception. Once a list is declared, a check surfaces a value off it with the list beside it and refuses nothing, so a blueprint carrying an old value is reported, not broken; surface those values before the owner narrows a list, the way any retired value is. 5.2.0 also fixes the name of the one file in a tool's `plugins/<name>/` folder that belongs to one person on one machine, `local.yaml`, and rules that it is never committed; everything else in the folder is the root's, as it was. The root's `.eidos/.gitignore` carries the pattern `plugins/*/local.yaml` beside `me.md`, so one line covers every tool and no tool writes a `.gitignore` of its own. No earlier root has such a file, since the standard had left personal state to each tool to name, so the whole hop is that line: add `plugins/*/local.yaml` under `me.md` in `.eidos/.gitignore`, then bump the version. A tool that had been gitignoring a personal file under its own name, or with a `.gitignore` inside its folder, moves to `local.yaml` in its own release; until it does, what it wrote is carried across as found, and nothing is faulted. The standard reads neither file, so a check reports nothing new; what is a root setting and what is a personal one stays the tool's to declare.
