...
_eidos/
  ...
  Framework.md             # the framework document: version, naming, Top-Level, Collections, Schema
                           #   (or Framework.yaml / Framework.json: the same fields as data, plus the index)
...
The framework document: the one file describing the structure rather than any single blueprint. In its markdown form, frontmatter for the facts tooling parses, and a body indexing what it governs. The same document may be written as data instead; see [`Framework.yaml` and `Framework.json`](#frameworkyaml-and-frameworkjson).
...
The framework document may be data instead of markdown: `Framework.yaml` (or `.yml`) or `Framework.json` in place of `Framework.md`, and exactly one of the three. It is the same framework, field for field, in the snake_case the frontmatter already uses; the markdown form's prose has no field to land in and stays behind. Markdown remains the recommended form, because it reads in a vault and beside the blueprints. The structured forms are for a root that tools read more than people do, and they carry one thing the markdown form keeps elsewhere: the generated index, under `index` (see [Generated leaves](#generated-leaves)), so that a structured root is one document with everything in it.
