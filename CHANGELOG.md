...
- **`_eidos/user.md` → `_eidos/me.md`.** "Me" cannot collide with anything, which `actor.md` would have: the standard itself offers `actor` as a role a film framework might declare, so `_eidos/roles/actor.md` beside `_eidos/actor.md` was a trap. The **concept is still the actor** — only the filename moved, the way `README.md` is the door while "Framework" is the concept.
