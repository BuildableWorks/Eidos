...
## [5.3.1] - 2026-09-18 — Standard: unchanged
...
- **The seeds are publishable to the Eidos Registry by path.** A Registry package is a manifest pointing at a root in its author's repository at a git tag, and at `v5.3.0` the seeds are still flat, so this tag is the first one a manifest can point at: `repo: BuildableWorks/Eidos`, `path: seeds/software` (or `seeds/book`, `seeds/research`), `ref: v5.3.1`.
