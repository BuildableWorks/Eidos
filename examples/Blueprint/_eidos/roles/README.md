...
These are the opinionated baseline, browsable here and installed into a definition's `_eidos/roles/` by `install` (committed, so a team can tune how a role is treated for their product). Each person who works on the definition picks one in their personal, gitignored `_eidos/me.md` and **calibrates** it — what they own on this product, their experience with the scope, and their technical capacity — with the `whoami` skill. Role sets the baseline; calibration tunes it per person.
