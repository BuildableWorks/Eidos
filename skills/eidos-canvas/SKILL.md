...
2. **Ask what to map** with `AskUserQuestion` — offer the declared collections (multi-select), default to all. A canvas can be the **whole definition** or a focused slice (`Specs`, or `Specs` plus `Frames`). `Frames` is a collection like any other; include or omit it.
3. **Propose a color schema.** Each collection gets its own color. Look at the definition — its collections (and, for `Specs`, the `type` values in play) — and propose a sensible mapping of one Obsidian preset per collection (1 red, 2 orange, 3 yellow, 4 green, 5 cyan, 6 purple); show it to the owner ("Frames = purple, Specs = green — look good?") and adjust to taste. **Skip this when a canvas already exists** at the output path: regenerating reuses its colors, so the owner's earlier choice sticks — don't re-ask.
...
python3 <skill>/build-canvas.py <definition-root> --collection Specs      # one collection (repeatable)
python3 <skill>/build-canvas.py <definition-root> --include-dependencies  # also draw depends_on edges (purple)
python3 <skill>/build-canvas.py <definition-root> --color Frames:6 --color Specs:4   # the agreed color schema
python3 <skill>/build-canvas.py <definition-root> --collection Specs --collection Frames --out "Framework Map.canvas"
```
