...
- [README](../README.md) — the definition's front door: what this programme asks, and pointers in.
<!-- One bullet per top-level document. README is the door and comes first; add your own below (a
     Protocol, a Data Statement, the generated Framework Map canvas), each a link and a one-line
     description. The framing docs live in the Frames collection, not here. eidos-configure refreshes
     this list. -->

## Collections

A collection is a top-level folder of repeated items that share a body shape. `Frames` holds the
framing docs — the most primary thing the definition says about itself — and `Investigations` the
programme's units. Add more with `eidos-configure`. Each lists its flavors (the default marked), how
...
Obsidian vault. The `eidos-configure` skill edits this section.

### Eidos Core

_Present on every item. Managed by the standard (Eidos 4.3.0); `eidos-migrate` rewrites this block on a version change — don't hand-edit it. (`flavor` absent = the collection's default; `connects_to` absent = no canvas edges; a missing `summary` is flagged by the index.)_
...
| strand        | Text | Investigations | The grouping — the line of inquiry this belongs to, matching the item's sub-folder in the naming convention. An unknown value warns, never fails. |
