...
It scaffolds collections and flavors, grows and reshapes the custom Schema and reconciles blueprints to it, and refreshes the Top-Level index. For anything the rules decide — what a collection is, the flavor model, the `flavor` property — defer to **EIDOS.md**.
...
The Framework is a current index and contract for the folder. From here, `eidos` reads it to know a blueprint's collection and flavors when authoring, and validates each blueprint against the updated Schema — a custom property now counts among the fields it checks for the collections it applies to, surfaced and added with a note where an applicable blueprint is missing it, never failing the file. `index` rebuilds each collection's `index.md` (the per-blueprint leaf) beneath it; and `README.md` is the visible door a human lands at first.
