...
- **The Framework body only.** You edit its `## Top-Level`, `## Collections`, and `### Custom Properties` sections, and create shape files in `_eidos/shapes/`. Not per-item `index.md` files (`eidos-index`), not items (`eidos`).
- **Never touch `### Eidos Core`.** Those move with the standard's version (`eidos-migrate`). A core property change is a standards change; redirect.
- **Needs a framework.** Read `_eidos/Framework.md` from the definition root, found by its `_eidos/` marker. No `_eidos/` means no framework installed — offer `eidos-install` first.
...
4. **Register it** under `## Collections` in `Framework.md`: a `###` heading, the description, then bullets — **Leaf**, **Flavors** (default marked), **Canvas**, and the grouping (sub-folders each with a short description, or "ungrouped"). Bullets, never `·` separators, so someone adding a flavor can copy a line:
   ...
   The **Canvas** bullet is the only thing telling `eidos-canvas` how this collection draws — it knows no collection by name.
...
6. **Build the leaf and hand off.** Run `eidos-index` for the new `index.md`, point the owner to `eidos` for the first item, and report the folder, shape file, and Collections entry.
...
2. **Rebuild the list** under `## Top-Level`, after the `<!-- eidos-configure: top-level index (regenerated) -->` marker: one bullet per doc, `- [Title](../Title.md) — one-line description`, `README` first. **Keep the owner's existing descriptions**; give a doc with none a `<!-- TODO: describe -->` and ask. Never invent one.
