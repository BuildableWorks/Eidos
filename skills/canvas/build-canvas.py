...
matching its sibling `index/build-index.py`. On a sandboxed host the skill falls
...
def canvas_filename(framework_md):
    """The canvas file's own name, in the framework's naming convention.

    The canvas is a top-level document, so its own name follows the same convention as
    everything else a human reads in the tree."""
...
def main():
    ...
    out_file = Path(args.out).resolve() if args.out else (vault_root / canvas_filename(framework_md))
