...
1. **Find the personas.** Read `_eidos/personas/` from the definition root, found by its `_eidos/` marker. No `_eidos/` means no framework installed — offer `install`. An `_eidos/` with no `personas/` is an older framework — offer to install a seed's, or point to `migrate`.
...
- **Personal file only.** You write `_eidos/user.md` — not items (`eidos`), not the framework index (`configure`), not the persona files themselves (a team decision).
