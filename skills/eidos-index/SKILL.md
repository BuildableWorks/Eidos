...
python3 <skill>/build-index.py <definition-root> --collection <Collection>   # limit to one collection (repeatable)
```
...
- Each collection's items live under `<Collection>/`, optionally in one level of sub-folders (`<Collection>/<Group>/`).
...
5. **Rewrite `<Collection>/index.md` wholesale.** It is fully generated — no hand-written prose to preserve — so rebuild the whole file:
   - An H1 of the collection name and the marker comment `<!-- eidos-index: <Collection> (regenerated) -->`.
   - **Grouped collection:** one `##` per sub-folder, then a bullet per item — `- [Title](<Sub-folder>/<File>.md) — summary`. Links are **relative to the collection folder** (`Identity/Magic%20Link%20Sign-In.md`, not `<Collection>/<Group>/…`), built in the framework's naming convention (read `naming` from `_eidos/Framework.md`): encode spaces as `%20` in a Title Case definition; a TitleCase or kebab-case one has none.
...
6. **Reconcile against the Framework.**
   - A sub-folder with items but **no description in the Framework** — note it and ask the owner to add one to the Collections section; don't invent it.
   - A sub-folder described in the Framework but **with no items** — dangling; flag it.
7. **Report** — the collections and items indexed, any items still missing a `summary` (and where), and any sub-folder still needing a description in the Framework.
...
- This is the leaf half of Eidos navigation; the top index is `_eidos/Framework.md` (Top-Level documents + Collections), kept by `eidos-configure`, with the visible root `README.md` as its door. The property/shape form lives in `_eidos/` and is handled by the other skills.
