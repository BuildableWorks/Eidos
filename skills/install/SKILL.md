...
A committed copy of `seeds/` ships in this folder. Each seed is a root laid out as an install leaves it: `.eidos/` (`templates/`, `roles/`, `Framework.yaml`, `me.md`, `.gitignore`), its declared folders (empty), and a `README.md`. Read them at runtime rather than trusting a table: `eidos seeds` prints every seed's version and folders in one call; by hand, read each seed's `.eidos/Framework.yaml` `folders` key.
