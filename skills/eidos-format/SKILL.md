...
- **A collection item** — a spec, or a Frame (Architecture, Audience, Criteria, Market) — reshape it toward its collection's flavor shape in `_eidos/shapes/` (the collection's default unless the draft's `flavor` says otherwise), with frontmatter from `_eidos/Framework.md`. The structured case: a spec follows its `spec.*` flavor, a frame its `frame.*` flavor.
...
2. **Get the form from the framework.** For a collection item (a spec or a frame), open its flavor shape in `_eidos/shapes/` — its collection's default unless its `flavor` says otherwise — and `_eidos/Framework.md` for the frontmatter contract. **A free-form top-level doc has no shape** — skip this and organize by the doc's own logic, keeping only the light frontmatter convention (`title`, `tags`, `date_created`, `date_modified`).
