...
`_eidos/user.md` is **personal and gitignored** — each person who works in the framework runs this for themselves, and no one's persona lands in anyone else's checkout.
...
1. **Find the personas.** Read `_eidos/personas/` from the framework root (found by its `_eidos/` marker). If there is no `_eidos/`, the framework isn't set up — offer `eidos-install`. If `_eidos/` exists but has no `personas/` (an older framework), offer to install them (from the standard's `personas/`), or point to `eidos-install`/`eidos-migrate`.
...
- **Never commit it.** `user.md` is gitignored by the framework's `.gitignore`. If it somehow isn't ignored, say so — it shouldn't be shared.
