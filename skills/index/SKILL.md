...
- This needs an installed framework. If there's no `_eidos/`, it isn't an Eidos definition yet — offer `install`.
...
5. **Rewrite `<Collection>/index.md` wholesale.** It is fully generated — no hand-written prose to preserve — so rebuild the whole file:
   ...
   - **Grouped collection:** one `##` per sub-folder, then a bullet per item — `- [Title](<Sub-folder>/<File>.md) — summary`. Links are **relative to the collection folder** (`identity/magic-link-sign-in.md`, not `<Collection>/<Group>/…`), built in the framework's naming convention (read `naming` from `_eidos/Framework.md`, kebab-case by default): encode spaces as `%20` in a Title Case definition; a kebab-case or TitleCase one has none.
