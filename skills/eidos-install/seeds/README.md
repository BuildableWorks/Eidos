...
The starting **frameworks** Eidos ships. A seed is a complete form layer — collections, body shapes and their flavors, personas, and a property Schema — that [`eidos-install`](../skills/eidos-install) copies into a new definition's `_eidos/`. Pick the one nearest what you're defining; reshape it from there with `eidos-configure`.
...
**None of these fit?** Start from the nearest and reshape it, or scaffold your own collections with `eidos-configure`. See [`examples/`](../examples) for two filled-in definitions to pattern-match against.
