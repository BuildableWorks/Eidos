...
Once the owner has answered the four questions eidos-install asks — which seed, which
...
Usage:
  ...
  --naming    Title Case (default) | TitleCase | kebab-case. Governs every human-facing
...
NAMINGS = ("Title Case", "TitleCase", "kebab-case")
DECLARED_BULLETS = ("leaf", "flavors", "canvas")  # every other bullet label is the grouping
INDEX_MARKER = "<!-- eidos-index: {name} (regenerated) -->"
...
def install(args):
    ...
    if (root / "_eidos").exists():
        print(f"error: {root}/_eidos already exists — that's a definition; use eidos-migrate", file=sys.stderr)
        return 2
    ...
    if args.group:
        if not grouped:
            ...
            return 2
        text = set_groups(text, grouped["grouping"], args.group)
    ...
    print(f"  collections: {', '.join(c['folder'] for c in collections)}"
          + (f"; groups under {grouped['folder']}: {', '.join(args.group)}" if args.group and grouped else ""))
    if scaffolded:
        print(f"  blank frames: {', '.join(scaffolded)}")
        print("  next: run eidos-index's build-index.py to list them in the leaf")
...
def main():
    ...
    ap.add_argument("--naming", choices=NAMINGS, default="Title Case", help="naming convention")
