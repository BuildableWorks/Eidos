...
1. Read the root's `.eidos/Framework.yaml` and check the frontmatter against _it_, every block of the Properties table — the core properties present and well-formed, plus the custom properties and any tool's own (`properties.tools.<tool>`) scoped to the blueprint's collection. A property entry's keys past the standard's four are a tool's; read the four and ignore the rest. A missing core field is surfaced and added with a note on why, never the file refused.
