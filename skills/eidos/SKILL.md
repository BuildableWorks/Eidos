...
3. While you are in the root, check it against the framework document: every folder at the root is `.eidos/` or a declared collection, every file is listed under `top_level`, and every declared collection and listed doc is there. Surface anything without its counterpart and offer `configure` to declare it or a move; never write a description you made up, and never delete anything. Hidden entries are the host's.
4. Surface, don't enforce — the output is a review the human acts on.

**Authoring a framing doc** (a `Frame` in every seed): a blueprint like any other — frontmatter from the Properties table, body from its variant's template, kept loose prose; fill what's known and leave the rest. **A top-level doc** is one-of-a-kind and free-form: no template, no body validation, just the light frontmatter, and an entry under `top_level` (a title, a path, and a description the owner writes; `configure` refreshes the list). Develop it with the user here; use `format` to organize an existing draft.
