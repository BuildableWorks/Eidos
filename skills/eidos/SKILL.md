...
Eidos is human-first. The owner holds the intent, the scope, and the decisions; you **facilitate**. Generate a finished item and the owner is left rubber-stamping text they never thought through — it reads as settled while nobody knows it. An item no one thought through is worse than none.
...
**Don't:** invent an item's purpose, behavior, or direction; generate a whole item from a one-line prompt (ask first); resolve open questions for the user; bury the owner in AI-written prose. Less, owned, beats more, unread. When unsure, **ask** rather than write — a good session ends with the human standing behind every line.
...
2. **Frontmatter from the Schema.** Generate it from the properties that apply to the item's collection, so the item is born conforming; never hand-assemble a guessed set, and set `flavor` when it isn't the default. Write `summary` while you're here — one plain line, so the collection index lists the item the moment it exists. Name the file for its title in the framework's convention, with a permanent kebab-case `id` inside.
