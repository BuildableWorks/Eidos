...
name: migrate
description: >-
  Migrate an Eidos folder — its blueprints and top-level docs — from one version of the standard to another. Use whenever someone wants to upgrade, migrate, or bring an Eidos folder up to date with a new Eidos version — e.g. "migrate our specs to Eidos 3.0", "we bumped the standard, update the specs", "bring this folder to the latest format", or "what changed between Eidos v1 and v3 and how do I move my specs over". Migrations are non-sequential: go directly from any source version to any target (v1.0.0 → v3.0.0) by diffing the two version snapshots. Trigger even when the user doesn't say "migrate" — "these specs are on the old format" or "update the frontmatter to the new schema" apply too.
---
...
- **`seeds/`** — every seed, for when a pre-v3 folder needs a form layer installed.
...
2. **Establish the source.** Check for the form-layer directory — `_eidos/` (v4.1+) or `.eidos/` (v3.0–v4.0) — and read its index file — `Framework.md` (v4.2+) or `Registry.md` (v3.0–v4.1) — where a v3+ folder declares its version. If there is neither (pre-v3), detect the source from the file shape and confirm with the user. Fingerprints:
   ...
   - **v3.0–v4.0** — has a form-layer directory named **`.eidos/`** (`shapes/`, `Schema.md`, `Registry.md`); the version is in `Registry.md`; blueprints carry no `eidos_version`. **v4.1** is identical but the directory is renamed **`_eidos/`** (the dot dropped so Obsidian shows it). **v4.2+** additionally renames the index file `Registry.md` → **`Framework.md`**; the `registry-owner` role is `framework-owner`. **v4.2.1** settles the vocabulary: the `_eidos/` form layer is the *framework*, the product written with it is the *folder*. Prose only. **v4.3.0** adds a `- **Canvas:**` bullet to each collection in `Framework.md`; an Eidos folder without one still loads, and draws every collection as a plain card. **v4.4.0** renames `_eidos/personas/` → `_eidos/roles/` and `_eidos/user.md` → `_eidos/me.md` (an Eidos folder still on the old names is pre-4.4.0), and flips the naming default: an absent `naming` key means `kebab-case` where it used to mean `Title Case`. An Eidos folder carrying the key is unaffected; for one without it, read the convention off its own filenames (a space means Title Case, space-free and capitalized means TitleCase, lowercase and hyphenated means kebab-case), confirm it, and write it down before bumping.
