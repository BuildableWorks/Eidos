...
5. **Apply, once the plan is agreed.** Order matters when the structure layer is involved:
   ...
   - **Set the version.** Write the target version into the structure dir's index file — `Framework.md` for 4.2+, `Registry.md` for 3.0–4.1 (creating it if new), under whichever directory name the target uses.
