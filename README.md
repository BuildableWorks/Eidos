...
> **[Eidos v4.2.1](EIDOS.md)** — the full standard.

A markdown standard for defining what a product _is_. One file completely defines one unit of the product — a **spec**, in the common case — true whether or not the thing has been built. The files live as plain `.md` next to your code. No SaaS. No lock-in. No hidden state.
...
Eidos turns on two words. A **framework** is the *form*: the collections, shapes, personas, naming convention, and property Schema that govern how you write. A **definition** is the *product*: the frames, items, and docs a team writes with a framework. One framework, many definitions — the framework is the portable piece, the part one team can hand to another.

A definition is a `Blueprint/` folder you drop into any repo. Everything Eidos defines is built from a few pieces:

```txt
Blueprint/                 # the definition — one product, defined
...
- **Framework** — the form layer, located by its hidden `_eidos/` folder, and the unit you can publish or hand to another team. [`_eidos/Framework.md`](seed/Framework.md) is its **index and config**: the Eidos version, the naming convention, the list of top-level docs, and the collections. A visible `README.md` is the door into it.
...
- **Personas & the actor** — [`_eidos/personas/`](seed/personas) say how the agent should talk to each kind of person (a designer gets experience terms; a developer, full technical depth); the personal, gitignored `_eidos/user.md` says who _you_ are, so the same definition answers each reader differently.
...
The full rules are in **[EIDOS.md](EIDOS.md)**. See **[`example/`](example/)** for a filled-in definition (a small subset of YouTube) to pattern-match against.
...
Doing Eidos needs the skills and a seeded definition: `EIDOS.md` gives the method, but the framework a definition uses lives in its `_eidos/`. See [`example/`](example/) for a filled-in definition to pattern-match against.
...
- **`eidos-install`** — scaffold a new definition (installs a framework into its `_eidos/`)
...
The **[`seed/`](seed)** folder is the public, browsable baseline that `eidos-install` installs into a definition's `_eidos/`. It is the standard's default framework — every piece of the form layer, laid out exactly as it lands in a fresh definition. Its contents:
...
- **`README.md`** — the `{{Product}}` front-door template that installs to the definition's root `README.md`, the human "start here."
