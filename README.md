...
A **framework** is one product's definition — a `Blueprint/` folder you drop into any repo. Everything Eidos defines is built from a few pieces:

```txt
Blueprint/                 # the framework — one product's definition
  README.md                # the human "start here"
  _eidos/                  # the form layer (hidden) — how this framework is shaped
    shapes/                #   collection body shapes, one per flavor (spec.*, frame.*)
    personas/              #   how the agent should talk to each role
    Framework.md            #   the index + config: version, naming, top-level docs, collections, and the property Schema
...
- **Framework** — the whole product definition, located by its hidden `_eidos/` folder. [`_eidos/Framework.md`](seed/Framework.md) is its **index and config**: the Eidos version, the naming convention, the list of top-level docs, and the collections. A visible `README.md` is the door into it.
...
- **Top-level docs** — one-of-a-kind documents you add at the root: a Roadmap, a Vision, the generated Framework Map. Free-form prose, no shape, no validation — the loose layer that annotates the whole.
- **Collections** — folders of repeated units (`Specs`, `Frames`, or your own). A framework can have several, and each can group its units one level deep in sub-folders. A unit is a small **frontmatter** contract (the firm part) plus a **body** (guidance).
...
- **Personas & the actor** — [`_eidos/personas/`](seed/personas) say how the agent should talk to each kind of person (a designer gets experience terms; a developer, full technical depth); the personal, gitignored `_eidos/user.md` says who _you_ are, so the same framework answers each reader differently.
...
The full rules are in **[EIDOS.md](EIDOS.md)**. See **[`example/`](example/)** for a filled-in framework (a small subset of YouTube) to pattern-match against.
...
3. **Fill the Frames.** Architecture, Audience, Criteria, Market — the `Frames` collection, prose, loose, point-in-time; fill what's known, leave the rest. Describe each Specs domain in the Framework's Collections section as specs accrue; each collection's `index.md` is generated by `eidos-index`.
4. **Author specs.** One file per unit under `Specs/<Domain>/`, named for its title in Title Case (`Magic Link Sign-In.md`). Each spec's frontmatter is generated from the framework's Schema; lead with Intent and Behaviors & Acceptance Criteria; press hard on **Out of Scope** — that's where scope is held. The `eidos` skill facilitates; it does not author for you.
...
Doing Eidos needs the skills and a seeded framework: `EIDOS.md` gives the method, but the form a framework uses lives in its `_eidos/`. See [`example/`](example/) for a filled-in framework to pattern-match against.
...
- **`eidos-install`** — scaffold a new framework (installs the `_eidos/` form layer)
...
No build step: each skill carries the committed copies it needs and reads a framework's form from its own `_eidos/` — all committed, so it behaves the same wherever it's installed.
...
A skill is just a folder with a `SKILL.md`. Drop the folder at `<repo>/.claude/skills/<name>/` (one project) or `~/.claude/skills/<name>/` (everywhere); a project copy wins over a global one. Each folder is self-contained — the runtime skills read your framework's `_eidos/`, and `eidos`/`eidos-install`/`eidos-migrate` carry their committed copies of the standard — so the folder works as-is.

**Adding your own skill:** create `skills/<your-skill>/SKILL.md` — it ships with the plugin automatically.

## Canonical Seed

The **[`seed/`](seed)** folder is the public, browsable baseline that `eidos-install` installs into a framework's `_eidos/`. It's the standard's opinionated starting point — every piece of the form layer, laid out exactly as it lands in a fresh framework. Its contents:
...
- **`Framework.md`** — the framework's index and config: the Eidos version, the naming convention, the Top-Level documents, the Collections (with their flavors and grouping), and the property **Schema** (the frontmatter contract, in its `## Schema` section).
...
- **`README.md`** — the `{{Product}}` front-door template that installs to the framework's root `README.md`, the human "start here."

It's a starting point, not a cage: a framework may reshape or override any of it — add a property, adjust a shape, add a flavor, retune a persona — without forking the standard. The default works out of the box; the parts you change are yours.
