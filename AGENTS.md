...
- **`eidos-install`** — scaffold a new framework
...
**The form lives in the framework.** A v4 framework owns its shapes, personas, and property contract in a hidden `_eidos/` (`shapes/` — collection body shapes, one or more flavors each, including the `Frames` collection's `frame.*` flavors; `personas/` — the response contracts per role; `Framework.md` — the framework's index of top-level docs and collections **and** the property Schema, in a `## Schema` section; and the personal `user.md`). The framing docs (Architecture, Audience, Criteria, Market) are the `Frames` collection, not templates — Eidos v4.1 retired the `templates/` concept, promoting them to a collection whose flavors are their shapes. The canonical defaults live, public and front-facing, at the top level in [`seed/`](seed) (personas included); `eidos-install` installs it into a framework's `_eidos/`, and the other skills read the form from the framework they're working in, not from a copy of their own. A framework's `_eidos/` is committed, never gitignored — except the personal `_eidos/user.md`, which the seeded `.gitignore` keeps out. A framework also records its naming convention (Title Case, TitleCase, or kebab-case) in `Framework.md`, and the skills locate a framework by its `_eidos/` marker — so the root folder may be named anything (`Blueprint` is just the default).
...
**Keep the version in sync.** `EIDOS.md` always holds the current version (its `**Version:**` header and Versioning section). When it changes, update: the badge at the top of `README.md` (e.g. `[Eidos v3.0.0](EIDOS.md)`); the `version` in `.claude-plugin/plugin.json`; both version spots in the canonical seed (`seed/Framework.md` — the `eidos_version` frontmatter **and** the version note in its `## Schema` block) and the example framework (`example/Blueprint/_eidos/Framework.md`); and `CHANGELOG.md`.
